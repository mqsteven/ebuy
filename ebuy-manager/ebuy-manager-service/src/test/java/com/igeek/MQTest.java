...
//		第一步：创建ConnectionFactory对象，需要指定服务端ip及端口号。
		ConnectionFactory connectionFactory = new ActiveMQConnectionFactory("tcp://192.168.128.150:61616");
...
//		第四步：使用Connection对象创建一个Session对象。
		...
		Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
//		第五步：使用Session对象创建一个Destination对象（topic、queue），此处创建一个Queue对象。
		Destination destination = session.createQueue("igeek-queue");//不要忘记设置队列的名称
//		第六步：使用Session对象创建一个consumer对象。
		MessageConsumer consumer = session.createConsumer(destination);
//		第七步：接收消息。通过consumer设置一个监听器，随时监听
		consumer.setMessageListener(new MessageListener() {//使用匿名内部类实现一个监听器
